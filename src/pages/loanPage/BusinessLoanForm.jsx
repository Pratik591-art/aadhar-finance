...
export default function BusinessLoanForm() {
  ...
  const validatePage1 = () => {
    ...
    if (!/^\d{15}$/.test(formData.gstNumber)) newErrors.gstNumber = 'GST Number must be 15 digits';
    if (!formData.shopPhoto) newErrors.shopPhoto = 'Shop Photo is required';
    if (!formData.lastSixMonthBill) newErrors.lastSixMonthBill = 'Last 6 Months Bills are required';
    if (!formData.gstDocument) newErrors.gstDocument = 'GST Document is required';
    if (!formData.shopStockPhoto) newErrors.shopStockPhoto = 'Shop Stock Photo is required';
    setErrors(newErrors);
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <h1 className="text-3xl font-bold text-gray-800">Business Loan Application</h1>
          </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none transition ${errors.gstNumber ? 'border-red-500' : 'border-gray-300 focus:border-blue-600'}`}
                  placeholder="15-digit GST number"
                  maxLength="15"
                />
                {errors.gstNumber && <p className="text-red-500 text-sm mt-1 flex items-center gap-1"><AlertCircle size={16} />{errors.gstNumber}</p>}
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Upload Shop Photo *</label>
                <div className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition ${errors.shopPhoto ? 'border-red-500 bg-red-50' : 'border-blue-300 hover:border-blue-600 bg-blue-50'}`}>
                  <input
                    type="file"
                    ...
                    accept="image/*"
                  />
                  <label htmlFor="shopPhoto" className="cursor-pointer flex flex-col items-center gap-2">
                    <Upload size={24} className="text-blue-600" />
                    <span className="text-sm font-medium">{formData.shopPhoto ? formData.shopPhoto.name : 'Click to upload Shop Photo'}</span>
                  </label>
                </div>
                {errors.shopPhoto && <p className="text-red-500 text-sm mt-1 flex items-center gap-1"><AlertCircle size={16} />{errors.shopPhoto}</p>}
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Upload Last 6 Months Bills *</label>
                <div className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition ${errors.lastSixMonthBill ? 'border-red-500 bg-red-50' : 'border-blue-300 hover:border-blue-600 bg-blue-50'}`}>
                  <input
                    type="file"
                    ...
                    id="lastSixMonthBill"
                  />
                  <label htmlFor="lastSixMonthBill" className="cursor-pointer flex flex-col items-center gap-2">
                    <Upload size={24} className="text-blue-600" />
                    <span className="text-sm font-medium">{formData.lastSixMonthBill ? formData.lastSixMonthBill.name : 'Click to upload Bills'}</span>
                  </label>
                </div>
                {errors.lastSixMonthBill && <p className="text-red-500 text-sm mt-1 flex items-center gap-1"><AlertCircle size={16} />{errors.lastSixMonthBill}</p>}
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Upload GST Document *</label>
                <div className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition ${errors.gstDocument ? 'border-red-500 bg-red-50' : 'border-blue-300 hover:border-blue-600 bg-blue-50'}`}>
                  <input
                    type="file"
                    ...
                    id="gstDocument"
                  />
                  <label htmlFor="gstDocument" className="cursor-pointer flex flex-col items-center gap-2">
                    <Upload size={24} className="text-blue-600" />
                    <span className="text-sm font-medium">{formData.gstDocument ? formData.gstDocument.name : 'Click to upload GST Document'}</span>
                  </label>
                </div>
                {errors.gstDocument && <p className="text-red-500 text-sm mt-1 flex items-center gap-1"><AlertCircle size={16} />{errors.gstDocument}</p>}
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Upload Shop Stock Photo *</label>
                <div className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition ${errors.shopStockPhoto ? 'border-red-500 bg-red-50' : 'border-blue-300 hover:border-blue-600 bg-blue-50'}`}>
                  <input
                    type="file"
                    ...
                    accept="image/*"
                  />
                  <label htmlFor="shopStockPhoto" className="cursor-pointer flex flex-col items-center gap-2">
                    <Upload size={24} className="text-blue-600" />
                    <span className="text-sm font-medium">{formData.shopStockPhoto ? formData.shopStockPhoto.name : 'Click to upload Shop Stock Photo'}</span>
                  </label>
                </div>
                {errors.shopStockPhoto && <p className="text-red-500 text-sm mt-1 flex items-center gap-1"><AlertCircle size={16} />{errors.shopStockPhoto}</p>}
              </div>
            </div>
          </div>
        )}
        ...
        {page === 2 && (
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-8 text-center">
              ...
              <p className="text-gray-600 mb-6">Your business information and documents have been received. Please proceed to verify your identity in the next step.</p>
              <div className="bg-white rounded-lg p-4 text-left space-y-2">
                ...
                <p className="text-sm"><span className="font-semibold">GST Number:</span> {formData.gstNumber}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {page === 4 && (
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <div className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-2xl p-12 text-center text-white">
              ...
              <p className="text-lg mb-8 opacity-90">Your business loan application is under review</p>
              ...
              <div className="space-y-3 text-left">
                <p className="text-sm opacity-90">✓ Business documents verified</p>
                <p className="text-sm opacity-90">✓ Identity confirmed</p>
                <p className="text-sm opacity-90">✓ GST details validated</p>
            ...
            <div className="mt-8 bg-blue-50 border-l-4 border-blue-600 p-4 rounded">
              <p className="text-gray-700 text-sm"><span className="font-semibold">Business Loan Reference ID:</span> BL-{Math.random().toString(36).substr(2, 9).toUpperCase()}</p>
            </div>
          </div>
        )}
